...
CONFIG = {
    ...
    "lr_lambda": 1e-4,       # Learning rate for the Lagrange multiplier
    "device": "cuda" if torch.cuda.is_available() else "cpu",
    "target_total_bits": 56.0, # Budget: 14 layers * 4 bits/layer
...
# 3. Budget-Aware FFN with Actor-Critic and Lagrangian Relaxation
...
def train(model, loader, optimizer, epoch, model_type, stats_log, **kwargs):
    ...
    if model_type == 'Budget-Aware AC':
        # Unpack kwargs specific to our new model
    ...
    for batch_idx, (data, target) in enumerate(pbar):
        ...
        if model_type == 'Budget-Aware AC':
            ...
            # Reward = -(TaskLoss) - lambda * (ResourceUsage)
            # The agent is rewarded for low task loss and low bit usage.
            with torch.no_grad():
                reward = -task_loss_per_sample - lagrangian_lambda * total_bits
            ...
            # 6. Update the Lagrange multiplier (lambda)
            # This is done via gradient ascent to enforce the budget constraint
            constraint_violation = total_bits.mean() - CONFIG['target_total_bits']
            lagrangian_lambda += CONFIG['lr_lambda'] * constraint_violation.item()
            lagrangian_lambda = max(0.0, lagrangian_lambda) # Ensure lambda is non-negative
            ...
            pbar.set_postfix(loss=f"{task_loss.item():.3f}", bits=f"{total_bits.mean().item():.1f}", λ=f"{lagrangian_lambda:.4f}")
    ...
    if model_type == 'Budget-Aware AC':
        return lagrangian_lambda # Return the updated lambda for the next epoch
...
def create_visualizations(all_stats):
    ...
    fig.suptitle('Deep FFN (15 Layers): FP32 vs. Static QAT vs. Budget-Aware AC', fontsize=22, weight='bold')
    ...
    # Plot lambda and average bits over training steps
    ax2 = axes[1,1].twinx()
    ...
    ax2.plot(iaq_stats['lambda_history'], 'r-', alpha=0.6, label='Lagrangian λ')
    axes[1,1].set_title("Budget-Aware AC: Bit Usage and λ vs. Training Steps"); axes[1,1].set_xlabel("Training Batches");
    axes[1,1].set_ylabel("Average Total Bits", color='g'); ax2.set_ylabel("Lambda (λ)", color='r')
    axes[1,1].legend(loc='upper left'); ax2.legend(loc='upper right'); axes[1,1].grid(True)
...
if __name__ == '__main__':
    ...
    # --- Training Loop ---
    lagrangian_lambda = 0.01 # Initial value for the Lagrange multiplier
    ...
    for name, model in models.items():
        ...
        for epoch in range(1, CONFIG['epochs'] + 1):
            if name == 'Budget-Aware AC':
                train_kwargs['lagrangian_lambda'] = lagrangian_lambda
                lagrangian_lambda = train(model, train_loader, optimizer, epoch, name, all_stats_logs[name], **train_kwargs)
            else:
