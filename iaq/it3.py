...
CONFIG = {
    ...
    "output_filename": "reflective_iaq_report.png",
    "hidden_dim": 128,
    "num_hidden_layers": 14,
    "lambda_bits": 0.0005,

    # --- NEW: Hyperparameter for the "Self-Reflection" mechanism ---
    "gamma_sensitivity": 0.05, # Weight for the gradient norm penalty in the reward
}
...
def train(model, loader, optimizer, epoch, model_type, stats_log, **kwargs):
    ...
    for batch_idx, (data, target) in enumerate(pbar):
        ...
        if model_type == 'IAQ':
            ...
            # --- "Reflective" Training Step ---
            # 1. Forward pass to get outputs for loss and policy
            student_logits, log_probs, bits_per_sample = model(data, temp=temp)
            
            # 2. Calculate per-sample loss. This is the primary error signal.
            task_loss_per_sample = F.cross_entropy(student_logits, target, reduction='none')

            # 3. "Self-Reflection": Calculate gradient norm as a sensitivity score.
            # This requires a gradient pass on the task loss.
            (grad_per_sample,) = torch.autograd.grad(
                outputs=task_loss_per_sample.sum(), # Sum losses to get total gradient signal
                inputs=student_logits,             # We want gradient w.r.t logits
                retain_graph=True,                 # CRITICAL: Keep graph for the main backward pass
                create_graph=False                 # We don't need higher-order gradients
            )
            sensitivity_score = torch.norm(grad_per_sample, p=2, dim=1)

            # 4. Formulate the "Reflective Reward" for the policy
            ...
            # 5. Calculate policy loss using the reflective reward
            policy_loss = (-log_probs * reward).mean()

            # 6. Final loss and update step
...
def create_visualizations(all_stats):
    ...
    fig.suptitle('Reflective IAQ (Gradient-Aware) vs. Baselines', fontsize=22, weight='bold')
...
if __name__ == '__main__':
    print(f"{C.HEADER}{C.BOLD}Starting Reflective IAQ Experiment (Gradient-Aware Policy){C.END}")
