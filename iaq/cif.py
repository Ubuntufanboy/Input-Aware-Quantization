...
class FakeQuantize(torch.autograd.Function):
    ...
    def forward(ctx, x, bits):
        ...
        dequantized = torch.round(x / (scale + 1e-9)) * scale
        return dequantized
    @staticmethod
...
# --- MODEL DEFINITIONS ---
# --- CIFAR-10 CHANGE: Models now accept a flexible input_dim ---
...
class QAT_DeepFFN(nn.Module):
    def __init__(self, input_dim, hidden_dim=128, output_dim=10, num_hidden_layers=14, static_bits=4):
        super().__init__(); self.static_bits = static_bits; self.layers = nn.ModuleList()
        self.layers.append(nn.Linear(input_dim, hidden_dim))
        for _ in range(num_hidden_layers - 1): self.layers.append(nn.Linear(hidden_dim, hidden_dim))
    ...
    def forward(self, x):
        ...
        for i, layer in enumerate(self.layers):
            ...
            x = F.linear(x, quantized_weight, layer.bias); x = F.relu(self.bns[i](x))
...
class QuantizedBlock(nn.Module):
    ...
    def forward(self, x, temp, is_eval=False):
        ...
        if is_eval:
            choice_indices = logits.argmax(dim=1); log_probs = None
        else:
...
class IAQ_DeepFFN(nn.Module):
    def __init__(self, bit_choices, input_dim, hidden_dim=128, output_dim=10, num_hidden_layers=14):
        super().__init__()
        # --- CIFAR-10 CHANGE: Correctly pass input_dim to the first block ---
...
# --- Training and Testing Loops (Identical logic, dataset-agnostic) ---
def train(model, loader, optimizer, epoch, model_type, stats_log, **kwargs):
    ...
    for batch_idx, (data, target) in enumerate(pbar):
        ...
        if model_type == 'IAQ':
            optim_policy = kwargs['optim_policy']; temp_schedule = kwargs['temp_schedule']
            ...
            pbar.set_postfix(loss=f"{main_task_loss.item():.3f}", reward=f"{reward.mean().item():.2f}", bits=f"{total_bits.mean().item():.1f}", sens=f"{sensitivity_score.mean().item():.2f}")
...
# --- Analysis & Visualization (Identical logic, updated titles) ---
def calculate_model_stats(model, model_type, iaq_stats=None):
    ...
    # GFLOPs calculation remains an approximation for FFNs
    gflops = 0
    for m in model.modules():
        if isinstance(m, nn.Linear): gflops += 2 * m.in_features * m.out_features
    stats['gflops'] = gflops / 1e9
    return stats

def create_visualizations(all_stats):
    fig, axes = plt.subplots(2, 2, figsize=(18, 14))
    # --- CIFAR-10 CHANGE: Updated plot title ---
    fig.suptitle('Reflective IAQ on CIFAR-10 vs. Baselines', fontsize=22, weight='bold')
    ...
    if 'IAQ' in all_stats:
        ...
        if losses and bits:
            ...
            axes[1,1].set_title("IAQ: Bit Usage vs. Input 'Difficulty' (Loss+Sensitivity)"); axes[1,1].set_xlabel("Per-Sample Loss"); axes[1,1].set_ylabel("Total Bits Used"); axes[1,1].grid(True)
...
if __name__ == '__main__':
    print(f"{C.HEADER}{C.BOLD}Starting Reflective IAQ Experiment on CIFAR-10{C.END}")
    print(f"Using device: {C.BOLD}{CONFIG['device']}{C.END}\n")

    all_stats_logs = defaultdict(lambda: defaultdict(list))
    models = {}
    
    # --- CIFAR-10 CHANGE: Instantiate models with the new input dimension ---
    ...
    for name, model in models.items():
        ...
        if name == 'IAQ':
            ...
            # Consider adding weight decay to the main optimizer for CIFAR-10 as it's a strong regularizer
            ...
            # Add weight decay to the baseline models as well for a fair comparison on CIFAR-10
